import { auth, db } from './firebase-config.js?v=seniorflow-react-20260630-stock-app-01';
...
const productoCoincide = (producto, query, proveedorElegido) => {
  ...
  if (proveedorElegido) {
    ...
    }) || camposProducto(producto).some((campo) => normalizarTexto(campo).includes(texto));
  }

  return camposProducto(producto).some((campo) => {
...
const renderResults = () => {
  ...
  matches.forEach((producto) => {
    ...
    button.innerHTML = `
      ...
      <span class="text-[11px] font-black text-emerald-700 bg-emerald-50 border border-emerald-100 rounded-full px-2 py-1">Elegir</span>
    `;
...
const actualizarProductoSeleccionado = async (event) => {
  ...
  try {
    ...
    renderStatus(`Listo. Nuevo stock: ${nuevoStock} ${productoSeleccionado.unidad || 'unid.'}`, 'ok');
    els.qtyInput.value = '1';
    els.nota.value = '';
    productoSeleccionado = { ...productoSeleccionado, ...payload };
...
const iniciarCamara = async () => {
  try {
    ...
    console.error(error);
    await detenerCamara();
    renderStatus('No pude abrir la camara. Revisa permisos y que la app este en HTTPS.', 'error');
  }
};
...
if ('serviceWorker' in navigator && window.location.protocol !== 'file:') {
  navigator.serviceWorker.register('./sw-stock-app.js?v=seniorflow-react-20260630-stock-app-01').catch(console.warn);
}
