...
const views = {
  ...
  presupuestos: { title: 'Presupuestos', subtitle: 'Presupuestos profesionales y exportables' },
...
const els = {
  ...
  expenseCategory: $('expenseCategory'), budgetSearch: $('budgetSearch'), budgetStatusFilter: $('budgetStatusFilter'), budgetsList: $('budgetsList'),
  budgetCustomerName: $('budgetCustomerName'), budgetValidity: $('budgetValidity'), budgetNote: $('budgetNote'), budgetStockProduct: $('budgetStockProduct'),
  budgetStockQty: $('budgetStockQty'), budgetManualName: $('budgetManualName'), budgetManualQty: $('budgetManualQty'), budgetManualUnit: $('budgetManualUnit'),
  budgetManualPrice: $('budgetManualPrice'), budgetDraftItems: $('budgetDraftItems'), budgetGrandTotal: $('budgetGrandTotal'), budgetGrandTotalWords: $('budgetGrandTotalWords')
};
...
function numberToWordsEs(value) {
  ...
  function underThousand(x) {
    ...
    return [hundreds[h], underHundred(r)].filter(Boolean).join(' ');
  }
  function section(x, divisor, singular, plural) {
    const count = Math.floor(x / divisor);
    const remainder = x - count * divisor;
    if (!count) return [ '', x ];
    if (count === 1) return [ singular, remainder ];
    return [ `${convert(count)} ${plural}`, remainder ];
  }
  function convert(x) {
    if (x === 0) return 'cero';
    let words = [];
    let rem = x;
    const [millions, remM] = section(rem, 1000000, 'un millón', 'millones'); rem = remM; if (millions) words.push(millions);
    const [thousands, remT] = section(rem, 1000, 'mil', 'mil'); rem = remT; if (thousands) words.push(thousands === 'mil' ? 'mil' : thousands);
    if (rem) words.push(underThousand(rem));
    return words.join(' ').replace(/\s+/g, ' ').trim();
  }
  const cents = Math.round((Math.abs(Number(value || 0)) - n) * 100);
  const sign = Number(value || 0) < 0 ? 'menos ' : '';
  return `${sign}${convert(n)} pesos${cents ? ` con ${String(cents).padStart(2, '0')}/100` : ''}`;
}
...
function attachMoneyPreview(input) {
  ...
  if (!preview) {
    ...
    preview.className = 'money-words muted';
...
function setMode(mode) {
  ...
  els.loginSubtitle.textContent = mode === 'administrador' ? 'Si no existe ningún administrador, el primer ingreso lo crea automáticamente.' : 'El vendedor entra con el usuario creado desde administración.';
}
...
function startRealtime() {
  ...
  listenCollectionAsc('products', (rows) => { state.products = rows; renderProducts(); renderCustomerSelects(); renderBudgetProductOptions(); });
...
function renderProducts() { const q = clean(els.productSearch.value); const filtered = state.products.filter((row) => `${clean(row.code)} ${clean(row.name)} ${clean(row.category)} ${clean(row.unit)}`.includes(q)); $('stockList').innerHTML = filtered.map((row) => `
    <div class="list-item">
      <div>
        <strong>${escapeHtml(row.name || 'Producto')}</strong>
        <small>${escapeHtml(row.code || 'Sin código')} · ${escapeHtml(row.category || 'Sin categoría')} · ${escapeHtml(row.unit || 'unidad')} · Stock ${Number(row.stock || 0)}</small>
      </div>
...
function renderBudgetDraftItems() {
  els.budgetDraftItems.innerHTML = state.budgetDraftItems.length ? state.budgetDraftItems.map((item, idx) => `
    <div class="list-item">
      <div>
        <strong>${escapeHtml(item.name)}</strong>
        <small>${Number(item.qty)} ${escapeHtml(item.unit)} · ${escapeHtml(item.source)}</small>
      </div>
      <div style="display:flex;gap:10px;align-items:center"><strong>${money(item.total)}</strong><button class="btn ghost" type="button" data-remove-budget-item="${idx}">Quitar</button></div>
    </div>
  `).join('') : '<p class="muted">Todavía no agregaste ítems al presupuesto.</p>';
  document.querySelectorAll('[data-remove-budget-item]').forEach((btn) => btn.addEventListener('click', () => { state.budgetDraftItems.splice(Number(btn.dataset.removeBudgetItem), 1); renderBudgetDraftItems(); updateBudgetTotalPreview(); }));
}

function renderBudgetProductOptions() {
  if (!els.budgetStockProduct) return;
  els.budgetStockProduct.innerHTML = '<option value="">Seleccionar producto de stock</option>' + state.products.map((row) => `<option value="${row.id}">${escapeHtml(row.name || 'Producto')} · ${escapeHtml(row.unit || 'unidad')} · ${money(row.price || 0)}</option>`).join('');
}
...
function budgetMarkup(budget) {
  ...
  const rows = items.map((item) => `
    <tr>
      <td style="padding:10px;border-bottom:1px solid #d6e0ef">${escapeHtml(item.name)}</td>
      <td style="padding:10px;border-bottom:1px solid #d6e0ef;text-align:center">${escapeHtml(String(item.qty))}</td>
      <td style="padding:10px;border-bottom:1px solid #d6e0ef;text-align:center">${escapeHtml(item.unit)}</td>
      <td style="padding:10px;border-bottom:1px solid #d6e0ef;text-align:right">${escapeHtml(money(item.price))}</td>
      <td style="padding:10px;border-bottom:1px solid #d6e0ef;text-align:right">${escapeHtml(money(item.total))}</td>
    </tr>
  `).join('');
  return `<!DOCTYPE html><html lang="es"><head><meta charset="UTF-8"><title>Presupuesto ${escapeHtml(budget.code || '')}</title></head><body style="font-family:Inter,Arial,sans-serif;color:#0f172a;margin:0;background:#f6f8fb"><div style="max-width:900px;margin:0 auto;padding:28px"><div style="background:#fff;border:1px solid #dbe4f0;border-radius:20px;overflow:hidden"><div style="padding:24px 28px;background:linear-gradient(135deg,#0b1220,#14233f);color:#fff"><div style="font-size:34px;font-weight:900;letter-spacing:.5px">${COMPANY.name}</div><div style="margin-top:8px;font-size:14px;opacity:.92">${COMPANY.email} · ${COMPANY.web} · ${COMPANY.whatsapp}</div></div><div style="padding:28px"><div style="display:flex;justify-content:space-between;gap:20px;flex-wrap:wrap"><div><div style="font-size:13px;color:#64748b;text-transform:uppercase;font-weight:700">Presupuesto</div><div style="font-size:30px;font-weight:900">${escapeHtml(budget.code || '—')}</div><div style="margin-top:10px;color:#334155">Fecha: ${escapeHtml(budget.date || '')}</div><div style="color:#334155">Estado: ${escapeHtml(cap(budget.status || 'borrador'))}</div></div><div><div style="font-size:13px;color:#64748b;text-transform:uppercase;font-weight:700">Cliente</div><div style="font-size:24px;font-weight:800">${escapeHtml(budget.customerName || '')}</div><div style="margin-top:10px;color:#334155">Validez: ${escapeHtml(budget.validity || '')}</div></div></div><div style="margin-top:24px;border:1px solid #dbe4f0;border-radius:16px;overflow:hidden"><table style="width:100%;border-collapse:collapse"><thead><tr style="background:#f8fbff"><th style="padding:12px;text-align:left">Ítem</th><th style="padding:12px">Cantidad</th><th style="padding:12px">Unidad</th><th style="padding:12px;text-align:right">Precio</th><th style="padding:12px;text-align:right">Subtotal</th></tr></thead><tbody>${rows}</tbody></table></div><div style="margin-top:24px;display:flex;justify-content:flex-end"><div style="min-width:280px;background:#f8fbff;border:1px solid #dbe4f0;border-radius:16px;padding:18px"><div style="font-size:13px;color:#64748b;text-transform:uppercase;font-weight:700">Total</div><div style="font-size:30px;font-weight:900;margin-top:6px">${money(budget.total || 0)}</div><div style="margin-top:8px;color:#475569">${escapeHtml(numberToWordsEs(budget.total || 0))}</div></div></div>${budget.note ? `<div style="margin-top:24px"><div style="font-size:13px;color:#64748b;text-transform:uppercase;font-weight:700">Observaciones</div><div style="margin-top:8px;color:#334155">${escapeHtml(budget.note)}</div></div>` : ''}<div style="margin-top:28px;padding-top:18px;border-top:1px solid #e2e8f0;color:#64748b;font-size:13px">Gracias por elegir ${COMPANY.name}. Este presupuesto fue generado desde SeniorFlow.</div></div></div></div></body></html>`;
}
...
async function exportBudgetPng(budget) {
  const canvas = document.createElement('canvas');
  canvas.width = 1400; canvas.height = 1800;
  const ctx = canvas.getContext('2d');
  ctx.fillStyle = '#f4f7fb'; ctx.fillRect(0,0,canvas.width,canvas.height);
  ctx.fillStyle = '#0b1220'; ctx.fillRect(60,60,1280,180);
  ctx.fillStyle = '#ffffff'; ctx.font = 'bold 64px Arial'; ctx.fillText(COMPANY.name, 100, 145);
  ctx.font = '28px Arial'; ctx.fillText(`${COMPANY.email} · ${COMPANY.web} · ${COMPANY.whatsapp}`, 100, 192);
  ctx.fillStyle = '#0f172a'; ctx.font = 'bold 44px Arial'; ctx.fillText(`Presupuesto ${budget.code || ''}`, 90, 320);
  ctx.font = '28px Arial'; ctx.fillText(`Cliente: ${budget.customerName || ''}`, 90, 370);
  ctx.fillText(`Fecha: ${budget.date || ''} · Estado: ${cap(budget.status || 'borrador')}`, 90, 412);
  let y = 500;
  ctx.font = 'bold 24px Arial'; ctx.fillText('Detalle', 90, y); ctx.fillText('Cant.', 760, y); ctx.fillText('Unidad', 900, y); ctx.fillText('Subtotal', 1140, y);
  y += 24; ctx.strokeStyle = '#d6e0ef'; ctx.beginPath(); ctx.moveTo(90, y); ctx.lineTo(1310, y); ctx.stroke(); y += 45;
  ctx.font = '24px Arial';
  for (const item of (budget.items || [])) {
    ctx.fillStyle = '#0f172a';
    ctx.fillText(String(item.name || '').slice(0, 42), 90, y);
    ctx.fillText(String(item.qty || ''), 770, y);
    ctx.fillText(String(item.unit || ''), 900, y);
    ctx.fillText(money(item.total || 0), 1140, y);
    y += 44;
    if (y > 1450) break;
  }
  ctx.strokeStyle = '#d6e0ef'; ctx.beginPath(); ctx.moveTo(90, y); ctx.lineTo(1310, y); ctx.stroke(); y += 70;
  ctx.font = 'bold 42px Arial'; ctx.fillText(`Total: ${money(budget.total || 0)}`, 840, y); y += 46;
  ctx.font = '24px Arial'; ctx.fillStyle = '#475569'; ctx.fillText(numberToWordsEs(budget.total || 0), 90, y);
  if (budget.note) { y += 90; ctx.fillStyle = '#0f172a'; ctx.font = 'bold 24px Arial'; ctx.fillText('Observaciones', 90, y); y += 38; ctx.font = '22px Arial'; ctx.fillStyle = '#334155'; ctx.fillText(String(budget.note).slice(0, 90), 90, y); }
...
function renderBudgets() {
  const search = clean(els.budgetSearch?.value);
  const status = els.budgetStatusFilter?.value || 'todos';
  const rows = state.budgets.filter((row) => {
    const matchesSearch = `${clean(row.code)} ${clean(row.customerName)} ${clean(row.note)}`.includes(search);
    const matchesStatus = status === 'todos' ? true : (row.status || 'borrador') === status;
    return matchesSearch && matchesStatus;
  });
  els.budgetsList.innerHTML = rows.length ? rows.map((row) => `
    <div class="list-item" style="grid-template-columns:1fr auto">
      <div>
        <strong>${escapeHtml(row.code || 'Presupuesto')}</strong>
        <small>${escapeHtml(row.customerName || '')} · ${escapeHtml(row.date || '')} · ${escapeHtml(cap(row.status || 'borrador'))} · ${escapeHtml((row.items || []).length)} ítems</small>
        <small>${escapeHtml(money(row.total || 0))}</small>
      </div>
      <div style="display:flex;gap:8px;flex-wrap:wrap;justify-content:flex-end">
        <button class="btn secondary" type="button" data-budget-print="${row.id}">PDF</button>
        <button class="btn secondary" type="button" data-budget-png="${row.id}">Imagen</button>
        <button class="btn secondary" type="button" data-budget-approve="${row.id}">${row.status === 'aprobado' ? 'Aprobado' : 'Aprobar'}</button>
        <button class="btn ghost" type="button" data-budget-delete="${row.id}">Eliminar</button>
      </div>
    </div>
  `).join('') : '<p class="muted">No hay presupuestos para ese filtro.</p>';
  document.querySelectorAll('[data-budget-print]').forEach((btn) => btn.addEventListener('click', () => { const budget = state.budgets.find((x) => x.id === btn.dataset.budgetPrint); if (budget) openBudgetForPrint(budget); }));
  document.querySelectorAll('[data-budget-png]').forEach((btn) => btn.addEventListener('click', () => { const budget = state.budgets.find((x) => x.id === btn.dataset.budgetPng); if (budget) exportBudgetPng(budget); }));
  document.querySelectorAll('[data-budget-approve]').forEach((btn) => btn.addEventListener('click', async () => { const budget = state.budgets.find((x) => x.id === btn.dataset.budgetApprove); if (!budget || budget.status === 'aprobado') return; await updateDoc(doc(db, 'budgets', budget.id), { status: 'aprobado', approvedAt: serverTimestamp(), updatedAt: serverTimestamp() }); }));
  document.querySelectorAll('[data-budget-delete]').forEach((btn) => btn.addEventListener('click', async () => { const budget = state.budgets.find((x) => x.id === btn.dataset.budgetDelete); if (!budget) return; if (!confirm(`Eliminar presupuesto ${budget.code || ''}?`)) return; await deleteDoc(doc(db, 'budgets', budget.id)); }));
}

async function getOrCreateCustomer(firstName, lastName, phone) {
  const fn = clean(firstName), ln = clean(lastName);
...
async function handleLogin(event) {
  ...
  try {
    ...
    const user = clean(els.username.value), pass = els.password.value;
...
async function submitProduct(event) { event.preventDefault(); const fd = new FormData(event.target); await addDoc(collection(db, 'products'), { code: fd.get('code') || '', name: fd.get('name'), category: fd.get('category') || '', unit: fd.get('unit') || 'unidad', stock: Number(fd.get('stock') || 0), cost: toNumber(fd.get('cost')), price: toNumber(fd.get('price')), updatedAt: serverTimestamp(), createdAt: serverTimestamp() }); event.target.reset(); els.productForm.classList.add('hidden'); bindMoneyInputs(); }
...
function addBudgetStockItem() {
  const product = state.products.find((row) => row.id === els.budgetStockProduct.value);
  const qty = Number(els.budgetStockQty.value || 0);
  if (!product || qty <= 0) return alert('Seleccioná un producto y una cantidad válida.');
  const item = { name: product.name || 'Producto', qty, unit: product.unit || 'unidad', price: Number(product.price || 0), total: qty * Number(product.price || 0), source: 'stock', productId: product.id };
  state.budgetDraftItems.push(item); els.budgetStockQty.value = 1; els.budgetStockProduct.value = ''; renderBudgetDraftItems(); updateBudgetTotalPreview();
}

function addBudgetManualItem() {
  const name = String(els.budgetManualName.value || '').trim(); const qty = Number(els.budgetManualQty.value || 0); const unit = els.budgetManualUnit.value || 'unidad'; const price = toNumber(els.budgetManualPrice.value);
  if (!name || qty <= 0) return alert('Completá el ítem manual y la cantidad.');
  const item = { name, qty, unit, price, total: qty * price, source: 'manual' };
  state.budgetDraftItems.push(item); els.budgetManualName.value = ''; els.budgetManualQty.value = 1; els.budgetManualUnit.value = 'unidad'; els.budgetManualPrice.value = ''; renderBudgetDraftItems(); updateBudgetTotalPreview(); bindMoneyInputs();
}

async function submitBudget(event) {
  event.preventDefault();
  if (!state.budgetDraftItems.length) return alert('Agregá al menos un ítem al presupuesto.');
  const fd = new FormData(event.target);
  const code = `PRES-${Date.now().toString().slice(-6)}`;
  const total = budgetTotal(state.budgetDraftItems);
  await addDoc(collection(db, 'budgets'), {
    code,
    date: fd.get('date'),
    customerName: fd.get('customerName'),
    note: fd.get('note') || '',
    items: state.budgetDraftItems,
    total,
    validity: fd.get('validity') || '7 días',
    status: 'borrador',
    createdBy: state.sessionUser.username,
    createdAt: serverTimestamp(),
    updatedAt: serverTimestamp()
  });
  state.budgetDraftItems = []; renderBudgetDraftItems(); updateBudgetTotalPreview(); event.target.reset(); setDates(); els.budgetValidity.value = '7 días';
}
...
function initExpensesCategories() {
  if (!els.expenseCategory) return;
  els.expenseCategory.innerHTML = EXPENSE_CATEGORIES.map((name) => `<option value="${escapeHtml(name)}">${escapeHtml(name)}</option>`).join('');
}
...
function initEvents() {
  ...
  $('addBudgetStockItemBtn')?.addEventListener('click', addBudgetStockItem);
  $('addBudgetManualItemBtn')?.addEventListener('click', addBudgetManualItem);
  ...
  els.budgetSearch?.addEventListener('input', renderBudgets);
  els.budgetStatusFilter?.addEventListener('change', renderBudgets);
}
...
async function boot() {
  ...
  setDates(); bindMoneyInputs(); setMode('administrador'); initInstall(); initPwa(); initExpensesCategories(); initEvents(); initAuth(); renderBudgetDraftItems(); updateBudgetTotalPreview(); showLogin();
