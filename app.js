...
function normalizeMoneyInput(raw) {
  ...
  let multiplier = 1;
  if (/(millon|millon(?:es)?|millones)/.test(original)) multiplier = 1000000;
  else if (/(mil)/.test(original)) multiplier = 1000;
  ...
  normalized = normalized.replace(/millones?|millon|mil/g, "");
