...
async function adminCount() {
  const snap = await getDocs(query(collection(db, 'users'), where('role', '==', 'administrador')));
  return snap.size;
}
...
function listenCollection(name, setter, sorterField = 'createdAt') {
  const ref = query(collection(db, name), orderBy(sorterField, 'desc'));
  unsubs.push(onSnapshot(ref, (snap) => setter(snap.docs.map((d) => ({ id: d.id, ...d.data() })) )));
}

function listenCollectionAsc(name, setter, sorterField = 'name') {
  const ref = query(collection(db, name), orderBy(sorterField, 'asc'));
  unsubs.push(onSnapshot(ref, (snap) => setter(snap.docs.map((d) => ({ id: d.id, ...d.data() })) )));
}
...
function onlyDate(row) {
  return String(row.date || '').slice(0, 10);
}

function amountByMethod(rows, method) {
  return rows.filter((row) => row.paymentMethod === method || row.method === method).reduce((sum, row) => sum + Number(row.amount || 0), 0);
}
...
function currentCash() {
  ...
  return {
    efectivo: sessionCash + salesCash + extraCash - expenseOut,
    cheques: sessionChecks + salesChecks + extraChecks
  };
}

function accountBalance(customerId) {
  return state.movements.filter((m) => m.customerId === customerId).reduce((sum, row) => sum + (row.type === 'cargo' ? Number(row.amount || 0) : -Number(row.amount || 0)), 0);
}
...
function weekBuckets() {
  ...
  state.sales.forEach((row) => {
    const d = new Date(`${onlyDate(row)}T12:00:00`);
    const day = (d.getDay() + 6) % 7;
    values[day] += Number(row.amount || 0);
  });
...
function monthBuckets() {
  ...
  state.sales.forEach((row) => {
    const d = new Date(`${onlyDate(row)}T12:00:00`);
    values[d.getMonth()] += Number(row.amount || 0);
  });
  return labels.map((label, idx) => ({ label, value: values[idx] }));
}
...
function card(title, subtitle, amount) {
  return `<div class="list-item"><div><strong>${escapeHtml(title)}</strong><small>${escapeHtml(subtitle || '')}</small></div>${amount ? `<strong>${escapeHtml(amount)}</strong>` : ''}</div>`;
}

function renderRecentMovements() {
  const rows = [
    ...state.sales.map((row) => ({ kind: 'Venta', text: `${row.documentType || 'Venta'} · ${row.paymentMethod || '-'}`, amount: row.amount, createdAt: row.createdAt })),
    ...state.cashAdjustments.map((row) => ({ kind: row.kind === 'ingreso' ? 'Ingreso caja' : 'Egreso caja', text: `${row.method} · ${row.detail || ''}`, amount: row.amount, createdAt: row.createdAt })),
    ...state.expenses.map((row) => ({ kind: 'Gasto', text: row.detail, amount: row.amount, createdAt: row.createdAt }))
  ].sort((a, b) => (b.createdAt?.seconds || 0) - (a.createdAt?.seconds || 0)).slice(0, 10);

  $('recentMovements').innerHTML = rows.length ? rows.map((row) => card(row.kind, row.text, money(row.amount))).join('') : '<p class="muted">Todavía no hay movimientos.</p>';
}

function renderBalances() {
  const rows = state.customers
    .map((customer) => ({ customer, balance: accountBalance(customer.id) }))
    .filter((item) => item.balance > 0)
    .sort((a, b) => b.balance - a.balance);

  $('balancesList').innerHTML = rows.length
    ? rows.map(({ customer, balance }) => card([customer.firstName, customer.lastName].filter(Boolean).join(' ') || customer.name || 'Cliente', customer.phone || 'Cuenta corriente', money(balance))).join('')
    : '<p class="muted">No hay saldos pendientes.</p>';
}

function renderSales() {
  $('salesList').innerHTML = state.sales.slice(0, 20).map((row) => {
    const detail = `${row.date || ''} · ${row.paymentMethod || '-'} · ${row.operationType || '-'}`;
    return card(row.documentNumber || 'Sin número', detail, money(row.amount));
  }).join('') || '<p class="muted">No hay ventas cargadas.</p>';
}

function renderExpenses() {
  $('expensesList').innerHTML = state.expenses.slice(0, 20).map((row) => card(row.category || 'Gasto', `${row.date || ''} · ${row.detail || ''}`, money(row.amount))).join('') || '<p class="muted">No hay gastos.</p>';
}

function renderProducts() {
  const q = clean(els.productSearch.value);
  const filtered = state.products.filter((row) => `${clean(row.code)} ${clean(row.name)} ${clean(row.category)}`.includes(q));
  $('stockList').innerHTML = filtered.map((row) => `
    <div class="list-item">
      <div>
...
function renderCustomers() {
  $('customersList').innerHTML = state.customers.map((row) => {
    const title = [row.firstName, row.lastName].filter(Boolean).join(' ') || row.name || 'Cliente';
    return card(title, row.phone || 'Sin teléfono', money(accountBalance(row.id)));
  }).join('') || '<p class="muted">No hay clientes.</p>';
}

function renderCustomerSelects() {
  els.paymentCustomerId.innerHTML = '<option value="">Seleccionar cliente</option>' + state.customers.map((row) => {
    const name = [row.firstName, row.lastName].filter(Boolean).join(' ') || row.name || 'Cliente';
    return `<option value="${row.id}">${escapeHtml(name)}</option>`;
  }).join('');
}

function renderBudgets() {
  $('budgetsList').innerHTML = state.budgets.map((row) => card(row.customerName || 'Cliente', `${row.date || ''} · Validez ${row.validity || '-'}`, money(row.total))).join('') || '<p class="muted">No hay presupuestos.</p>';
}

function renderUsers() {
  $('usersList').innerHTML = state.users.map((row) => card(row.name || row.username, row.role || '-', row.username || '')).join('') || '<p class="muted">No hay usuarios.</p>';
}

function renderCashHistory() {
  const rows = [
    ...state.cashSessions.map((row) => ({ title: 'Apertura de caja', subtitle: `${row.date} · ${row.note || ''}`, amount: money((row.cashAmount || 0) + (row.checkAmount || 0)) })),
    ...state.cashAdjustments.map((row) => ({ title: row.kind === 'ingreso' ? 'Ingreso de caja' : 'Egreso de caja', subtitle: `${row.date} · ${row.method} · ${row.detail || ''}`, amount: money(row.amount) }))
  ];
  $('cashHistory').innerHTML = rows.length ? rows.map((row) => card(row.title, row.subtitle, row.amount)).join('') : '<p class="muted">Todavía no hay historial de caja.</p>';
}
...
function requireAdminView() {
  ...
  if (!isAdmin() && document.querySelector('.nav-btn.active')?.dataset.view === 'usuarios') {
    switchView('inicio');
  }
}
...
async function handleLogin(event) {
  ...
  try {
    ...
    const user = clean(els.username.value);
    const pass = els.password.value;
    if (!user || !pass) throw new Error('Completá usuario y contraseña.');

    if (state.mode === 'administrador' && (await adminCount()) === 0) {
      await createFirstAdmin(user, pass);
    }

    await loginWithAppUser(user, pass);
    showApp();
    els.sessionMeta.textContent = `${state.sessionUser.name || state.sessionUser.username} · ${state.sessionUser.role}`;
    requireAdminView();
    startRealtime();
    switchView('inicio');
  } catch (error) {
    console.error(error);
    els.loginError.textContent = error.message || 'No se pudo ingresar.';
  }
}
...
async function submitOpenCash(event) {
  ...
  await addDoc(collection(db, 'cash_sessions'), {
    date: fd.get('date'),
    cashAmount: toNumber(fd.get('cashAmount')),
    checkAmount: toNumber(fd.get('checkAmount')),
    note: fd.get('note') || '',
...
async function submitAdjustment(event) {
  event.preventDefault();
  const fd = new FormData(event.target);
  await addDoc(collection(db, 'cash_adjustments'), {
    date: fd.get('date'),
    kind: fd.get('kind'),
...
async function submitSale(event) {
  event.preventDefault();
  const fd = new FormData(event.target);
  const operationType = fd.get('operationType');
  let customerId = '';
  if (operationType === 'cuenta_corriente') {
    customerId = await getOrCreateCustomer(fd.get('customerFirstName'), fd.get('customerLastName'), fd.get('customerPhone'));
  }
  ...
  await addDoc(collection(db, 'sales'), {
    ...
    customerId,
    checkBank: fd.get('checkBank') || '',
    checkNumber: fd.get('checkNumber') || '',
    checkHolder: fd.get('checkHolder') || '',
    checkDueDate: fd.get('checkDueDate') || '',
    createdBy: state.sessionUser.username,
    createdAt: serverTimestamp()
  });
  if (customerId) {
    await addDoc(collection(db, 'account_movements'), {
      ...
      type: 'cargo',
      amount,
      detail: `${fd.get('documentType')} ${fd.get('documentNumber')}`,
      createdBy: state.sessionUser.username,
      createdAt: serverTimestamp()
    });
  }
...
async function submitAccountPayment(event) {
  event.preventDefault();
  const fd = new FormData(event.target);
  await addDoc(collection(db, 'account_movements'), {
...
async function submitUser(event) {
  ...
  if (!isAdmin()) return;
  const fd = new FormData(event.target);
  const username = clean(fd.get('username'));
  await setDoc(doc(db, 'users', sanitizeId(username)), {
    name: fd.get('name'),
    username,
    passwordHash: await sha256(fd.get('password')),
    role: 'vendedor',
    createdAt: serverTimestamp(),
    updatedAt: serverTimestamp()
  }, { merge: true });
...
function toggleSaleFields() {
  ...
  const operationType = els.operationType.value;
  els.customerFields.classList.toggle('hidden', operationType !== 'cuenta_corriente');
  els.checkFields.classList.toggle('hidden', paymentMethod !== 'cheque');
}
...
async function boot() {
  ...
  try {
    ...
    console.error(error);
    els.loginError.textContent = 'No se pudo conectar con Firebase. Revisá dominio autorizado, conexión y reglas.';
  }
}
