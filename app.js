...
function renderSales() {
  const rows = sortNewestFirst(state.sales);
  if (!rows.length) {
...
if ('serviceWorker' in navigator) {
  window.addEventListener('load', async () => {
    try {
      ...
      regs.forEach((reg) => {
        if (!reg.active?.scriptURL?.includes('sf-final-20260406') && !reg.installing?.scriptURL?.includes('sf-final-20260406')) {
          reg.update();
        }
      });
      await navigator.serviceWorker.register('./sw.js?v=sf-final-20260406');
    } catch (error) {
      console.error(error);
    }
  });
