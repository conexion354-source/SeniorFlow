...
function setupInstallPrompt() {
  ...
  const updateInstallButton = () => {
    ...
    if (deferredPrompt) {
      ...
      els.installBtn.title = "Instalar Mundo Led Control";
      els.installBtn.classList.remove("hidden");
      return;
    }

    els.installBtn.textContent = isIOS ? "Cómo instalar en iPhone" : "Cómo instalar app";
    els.installBtn.title = "Ver ayuda de instalación";
    els.installBtn.classList.remove("hidden");
  };

  const showInstallHelp = () => {
    if (isIOS) {
      alert("Para instalar en iPhone: abrí Compartir (cuadro con flecha) y elegí 'Agregar a pantalla de inicio'.");
      return;
    }
    if (isAndroid) {
    ...
    alert("Para instalar en PC: abrí la app en Chrome o Edge y usá el ícono de instalación de la barra o el menú ⋮ > 'Instalar aplicación'.");
  };
  ...
  setTimeout(updateInstallButton, 300);
  updateInstallButton();
}
