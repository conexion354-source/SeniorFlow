...
const els = {
  ...
  budgetCustomerSelect: document.getElementById("budgetCustomerSelect"),
...
function fillCustomerSelects() {
  ...
  const html = ['<option value="">Seleccionar cliente</option>'].concat(
    customers.map((customer) => `<option value="${customer.id}">${escapeHtml(getCustomerFullName(customer))} · ${escapeHtml(customer.phone || "")}</option>`)
  ).join("");
  if (els.budgetCustomerSelect) els.budgetCustomerSelect.innerHTML = html;
}
      ...
      <tbody>
        ${rows.map((sale) => `
          <tr>
            ...
            <td>${escapeHtml(sale.documentType || "-")}</td>
            <td>${escapeHtml(sale.documentNumber || "-")}</td>
            <td>${sale.operationType === "cuenta_corriente" ? '<span class="tag">Cuenta corriente</span>' : '<span class="tag">Contado</span>'}</td>
            <td>${sale.operationType === "cuenta_corriente" ? escapeHtml(`${sale.customerFirstName || ""} ${sale.customerLastName || ""}`.trim() || "-") : "-"}</td>
...
function renderExpenses() {
  if (!state.expenses.length) {
    els.expensesTableWrap.innerHTML = '<div class="empty">No hay gastos registrados.</div>';
    return;
  }
...
function renderAccountDetail(customerId) {
  ...
  card.innerHTML = `
    ...
    <table class="data-table account-movements-table">
      <thead>
        <tr>
          <th>Fecha</th>
          <th>Tipo</th>
      ...
      <tbody>
        ${movements.length ? movements.map((item) => `
          <tr>
            ...
            <td>${item.type === "cobro" ? '<span class="status-pill success">Cobro</span>' : '<span class="status-pill warning">Cargo</span>'}</td>
            <td>${escapeHtml(item.details || "-")}</td>
            <td>${escapeHtml(item.paymentMethod || "-")}</td>
            <td>${currency(item.amount)}</td>
          </tr>
        `).join("") : '<tr><td colspan="5">No hay movimientos para este cliente.</td></tr>'}
      </tbody>
    </table>
  `;
}
...
function renderCash() {
  ...
  node.innerHTML = state.cashMovements.length ? state.cashMovements.slice(0, 30).map((item) => `
    <div class="simple-item">
      ...
      <div class="muted">${item.kind === "apertura" ? `Efectivo ${currency(item.cashAmount || 0)} · Cheques ${currency(item.checkAmount || 0)} · Otros ${currency(item.otherAmount || 0)}` : `${escapeHtml(String(item.medium || "otro"))} · ${escapeHtml(item.details || "")}`}</div>
    </div>
  `).join("") : '<div class="empty">Todavía no registraste apertura o ingresos manuales.</div>';
}
      ...
      <tbody>
        ${state.users.map((user) => `
          <tr>
            ...
            <td>${isAdmin() && user.id !== state.sessionUser.id ? `<button class="btn btn-danger user-delete-btn" data-id="${user.id}" type="button">Eliminar</button>` : ""}</td>
          </tr>
        `).join("")}
      </tbody>
    </table>
...
async function saveCustomerForAccount(data) {
  const key = sanitizeId(`${data.customerFirstName}_${data.customerLastName}_${data.customerPhone || data.customerEmail || "cliente"}`);
  const payload = {
...
function createBudgetItemRow(item = {}) {
  ...
  row.innerHTML = `
    ...
    <label>
      ...
      <select name="stockId" class="${source === "stock" ? "" : "hidden"}">${options}</select>
    </label>
    <label class="budget-description-wrap">
      <span>Descripción</span>
      <input name="description" type="text" value="${escapeHtml(item.description || "")}" placeholder="Detalle del producto o servicio" required />
    </label>
    <label>
      <span>Cantidad</span>
  ...
  if (source === "stock" && item.stockId) {
    ...
    if (product && !item.unitPrice) row.querySelector('[name="unitPrice"]').value = formatMoneyInputValue(product.salePrice || product.cost || 0);
    if (product && !item.unit) unitInput.value = product.unit || "unidad";
  }
...
function loadBudgetIntoForm(id) {
  ...
  if (budget.customerId) {
    ...
    els.budgetCustomerSelect.value = budget.customerId;
  } else {
    els.budgetClientMode.value = "manual";
...
function bindFilters() {
  ...
  [els.salesFilterFrom, els.salesFilterTo, els.budgetSearchFrom, els.budgetSearchTo].forEach((input) => input?.addEventListener("change", renderEverything));
}
...
function setupUiEvents() {
  ...
  els.budgetCustomerSelect?.addEventListener("change", () => {
    const customer = state.customers.find((item) => item.id === els.budgetCustomerSelect.value);
    if (!customer) return;
    const form = els.budgetForm;
...
async function saveSaleForm(event) {
  ...
  showMessage("saleMsg", state.editingSaleId ? "Venta actualizada correctamente." : "Venta guardada correctamente.");
  resetSaleForm();
}
...
async function saveExpenseForm(event) {
  ...
  showMessage("expenseMsg", state.editingExpenseId ? "Gasto actualizado correctamente." : "Gasto guardado correctamente.");
  resetExpenseForm();
}
...
async function saveProductForm(event) {
  ...
  showMessage("productMsg", state.editingProductId ? "Producto actualizado correctamente." : "Producto guardado correctamente.");
  resetProductForm();
}
...
async function saveBudgetForm(event) {
  ...
  if (els.budgetClientMode.value === "existing") {
    customerId = els.budgetCustomerSelect.value;
    const customer = state.customers.find((item) => item.id === customerId);
  ...
  showMessage("budgetMsg", state.editingBudgetId ? "Presupuesto actualizado correctamente." : "Presupuesto guardado correctamente.");
  resetBudgetForm();
}
...
async function saveUserForm(event) {
  ...
  form.reset();
  showMessage("userMsg", "Usuario guardado correctamente.");
}
