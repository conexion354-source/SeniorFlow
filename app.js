...
const els = {
  ...
  detailSalesDayBtn: document.getElementById('detailSalesDayBtn'),
  detailExpensesDayBtn: document.getElementById('detailExpensesDayBtn'),
  detailCashBtn: document.getElementById('detailCashBtn'),
  detailSalesMonthBtn: document.getElementById('detailSalesMonthBtn'),
  ...
  statCashDebitCard: document.getElementById('statCashDebitCard'),
  statCashCreditCard: document.getElementById('statCashCreditCard'),
  ...
  salesTodayBtn: document.getElementById('salesTodayBtn'),
  salesAllBtn: document.getElementById('salesAllBtn'),
  salesFilterSummary: document.getElementById('salesFilterSummary'),
...
const PAYMENT_METHODS = {
  efectivo: { label: 'Efectivo', cashBucket: 'efectivo' },
  cheque: { label: 'Cheque', cashBucket: 'cheque', usesReference: true },
  transferencia: { label: 'Transferencia', cashBucket: 'transferencia', usesReference: true },
  tarjeta_debito: { label: 'Tarjeta de débito', cashBucket: 'tarjetaDebito', usesReference: true },
  tarjeta_credito: { label: 'Tarjeta de crédito', cashBucket: 'tarjetaCredito', usesReference: true },
  cuenta_corriente: { label: 'Cuenta corriente', requiresCustomer: true },
};
...
const cashTotal = (breakdown) => CASH_BUCKETS.reduce((sum, bucket) => sum + Number(breakdown[bucket.key] || 0), 0);
const paymentMethodConfig = (method) => PAYMENT_METHODS[method] || { label: method || '-' };
const paymentMethodLabel = (method) => paymentMethodConfig(method).label;
const sumAmounts = (items) => items.reduce((sum, item) => sum + Number(item.amount || 0), 0);
const sumAmountsByDate = (items, date) => sumAmounts(items.filter((item) => item.date === date));
const sumAmountsBetweenDates = (items, from, to) => sumAmounts(items.filter((item) => item.date >= from && item.date <= to));
const sortNewestFirst = (items) => [...items].sort((a, b) => (b.date || '').localeCompare(a.date || '') || (b.createdAt?.seconds || 0) - (a.createdAt?.seconds || 0));
const sortOldestFirst = (items) => [...items].sort((a, b) => (a.date || '').localeCompare(b.date || ''));

function filteredSalesForList() {
  if (state.salesFilterMode === 'all') return sortNewestFirst(state.sales);
  return sortNewestFirst(state.sales.filter((sale) => sale.date === state.salesFilterDate));
}
...
function currentCajaBreakdown() {
  ...
  state.sales.filter((sale) => sale.cashSessionId === current.id).forEach((sale) => {
    const bucket = paymentMethodConfig(sale.paymentMethod).cashBucket;
    if (bucket) breakdown[bucket] += Number(sale.amount || 0);
  });
...
function renderDashboard() {
  ...
  const salesDay = sumAmountsByDate(state.sales, today);
  const expensesDay = sumAmountsByDate(state.expenses, today);
  const extraIncomeDay = sumAmountsByDate(state.cashMovements, today);
  const startOfMonth = `${today.slice(0, 7)}-01`;
  const salesMonth = sumAmountsBetweenDates(state.sales, startOfMonth, today);
  ...
  for (let i = 6; i >= 0; i -= 1) {
    ...
    const amount = sumAmountsByDate(state.sales, iso);
    last7.push({
      iso,
  ...
  els.statCashBreakdown.textContent = CASH_BUCKETS.map((bucket) => `${bucket.label} ${currency(box[bucket.key])}`).join(' · ');
  ...
  els.statCashDebitCard.textContent = currency(box.tarjetaDebito);
  els.statCashCreditCard.textContent = currency(box.tarjetaCredito);
...
function detailSummaryHtml(label, amount, count = null) {
  const countText = count === null ? '' : ` · ${count} registro${count === 1 ? '' : 's'}`;
  return `<div class="detail-summary">${escapeHtml(label)}: ${escapeHtml(currency(amount))}${escapeHtml(countText)}</div>`;
}

function emptyDetailHtml(text) {
  return `<div class="empty">${escapeHtml(text)}</div>`;
}

function salesDetailHtml(rows) {
  if (!rows.length) return emptyDetailHtml('No hay ventas para mostrar.');
  return `
      ...
      ${rows.map((sale) => `
        <article class="record-item">
          <div class="record-head">
            <div>
              <strong>${escapeHtml(currency(sale.amount))}</strong>
              <div class="record-meta">
                <span>${escapeHtml(dateLabel(sale.date))}</span>
                <span>${escapeHtml(sale.documentType || '-')} ${escapeHtml(sale.documentNumber || '')}</span>
                <span>${escapeHtml(sale.paymentMethodLabel || paymentMethodLabel(sale.paymentMethod))}</span>
              </div>
            </div>
          </div>
          <div class="record-meta">
            ${sale.bankInfo ? `<span>Referencia: ${escapeHtml(sale.bankInfo)}</span>` : ''}
            ${sale.details ? `<span>${escapeHtml(sale.details)}</span>` : '<span>Sin detalle</span>'}
          </div>
        </article>
      `).join('')}
    </div>
...
function expensesDetailHtml(rows) {
  if (!rows.length) return emptyDetailHtml('No hay gastos para mostrar.');
  return `
    <div class="detail-list">
      ${rows.map((expense) => `
        <article class="record-item">
          <div class="record-head">
            <div>
              <strong>${escapeHtml(currency(expense.amount))}</strong>
              <div class="record-meta">
...
function cashDetailHtml() {
  if (!state.currentSession) return emptyDetailHtml('No hay caja abierta.');
  const box = currentCajaBreakdown();
  const sessionId = state.currentSession.id;
  const openingRows = [
    { label: 'Apertura efectivo', amount: Number(state.currentSession.openingCash || 0) },
    { label: 'Apertura cheques', amount: Number(state.currentSession.openingChecks || 0) },
    { label: 'Apertura transferencias', amount: Number(state.currentSession.openingTransfers || 0) },
  ].filter((item) => item.amount);
  const saleRows = state.sales.filter((sale) => sale.cashSessionId === sessionId && paymentMethodConfig(sale.paymentMethod).cashBucket)
    .map((sale) => ({ label: `Venta ${sale.paymentMethodLabel || paymentMethodLabel(sale.paymentMethod)}`, amount: Number(sale.amount || 0), detail: `${sale.documentType || '-'} ${sale.documentNumber || ''}`.trim(), date: sale.date }));
  const movementRows = state.cashMovements.filter((move) => move.cashSessionId === sessionId)
    .map((move) => ({ label: `Ingreso a ${move.target || '-'}`, amount: Number(move.amount || 0), detail: move.sourceName || move.details || move.sourceTypeLabel || 'Ingreso', date: move.date }));
  const expenseRows = state.expenses.filter((expense) => expense.cashSessionId === sessionId)
    .map((expense) => ({ label: `Gasto ${expense.category || '-'}`, amount: -Number(expense.amount || 0), detail: expense.details || 'Gasto', date: expense.date }));
  const rows = [...openingRows, ...saleRows, ...movementRows, ...expenseRows];
  const breakdown = CASH_BUCKETS.map((bucket) => `<span>${escapeHtml(bucket.label)} ${escapeHtml(currency(box[bucket.key]))}</span>`).join('');

  return `
    ${detailSummaryHtml('Total en caja', cashTotal(box))}
    <div class="record-meta" style="margin-top:12px;">${breakdown}</div>
    <div class="detail-list">
      ${rows.length ? rows.map((item) => `
        <article class="record-item">
          <div class="record-head">
            <div>
              <strong>${escapeHtml(currency(item.amount))}</strong>
              <div class="record-meta">
                ${item.date ? `<span>${escapeHtml(dateLabel(item.date))}</span>` : ''}
      ...
      `).join('') : emptyDetailHtml('No hay movimientos en la caja actual.')}
    </div>
  `;
}
...
function openDashboardDetail(type) {
  ...
  const startOfMonth = `${today.slice(0, 7)}-01`;
  if (type === 'salesDay') {
    const rows = sortNewestFirst(state.sales.filter((sale) => sale.date === today));
    els.detailTitle.textContent = `Ventas del día ${dateLabel(today)}`;
    els.detailBody.innerHTML = detailSummaryHtml('Total ventas', sumAmounts(rows), rows.length) + salesDetailHtml(rows);
  }
  if (type === 'expensesDay') {
    const rows = sortNewestFirst(state.expenses.filter((expense) => expense.date === today));
    els.detailTitle.textContent = `Gastos del día ${dateLabel(today)}`;
    els.detailBody.innerHTML = detailSummaryHtml('Total gastos', sumAmounts(rows), rows.length) + expensesDetailHtml(rows);
  }
  if (type === 'cash') {
    els.detailTitle.textContent = 'Detalle de caja';
    els.detailBody.innerHTML = cashDetailHtml();
  }
  if (type === 'salesMonth') {
    const rows = sortNewestFirst(state.sales.filter((sale) => sale.date >= startOfMonth && sale.date <= today));
    els.detailTitle.textContent = `Ventas mensuales ${today.slice(5, 7)}/${today.slice(0, 4)}`;
    els.detailBody.innerHTML = detailSummaryHtml('Total ventas mensuales', sumAmounts(rows), rows.length) + salesDetailHtml(rows);
  }
...
function renderCashMovements() {
  const rows = sortNewestFirst(state.cashMovements);
  if (!rows.length) {
...
function renderSales() {
  const rows = filteredSalesForList();
  const total = sumAmounts(rows);
  const isAll = state.salesFilterMode === 'all';
  els.salesFilterDate.value = state.salesFilterDate;
  els.salesFilterSummary.textContent = isAll
    ? `Mostrando todas las ventas: ${rows.length} registros · ${currency(total)}`
    : `Mostrando ventas del ${dateLabel(state.salesFilterDate)}: ${rows.length} registros · ${currency(total)}`;
  if (!rows.length) {
    els.salesList.innerHTML = `<div class="empty">${isAll ? 'No hay ventas cargadas.' : 'No hay ventas cargadas para esta fecha.'}</div>`;
    return;
  }
...
function renderExpenses() {
  const rows = sortNewestFirst(state.expenses);
  if (!rows.length) {
...
function setSalesDateFilter(date = todayISO()) {
  state.salesFilterMode = 'date';
  state.salesFilterDate = date || todayISO();
  renderSales();
}

function setSalesAllFilter() {
  state.salesFilterMode = 'all';
  renderSales();
}
...
function updateSaleConditionalFields() {
  ...
  els.saleCustomerWrap.classList.toggle('hidden', !config.requiresCustomer);
  els.saleBankWrap.classList.toggle('hidden', !config.usesReference);
  const form = els.saleForm;
  form.customerFirstName.required = !!config.requiresCustomer;
  form.customerLastName.required = !!config.requiresCustomer;
}
...
function buildReport(period) {
  ...
  const rows = sortOldestFirst(state.sales.filter((item) => item.date >= from && item.date <= to));
  const total = sumAmounts(rows);
      ...
      <tbody>
        ${rows.map((sale) => `
          <tr>
            ...
            <td style="padding:8px; border-bottom:1px solid #eee;">${escapeHtml(sale.paymentMethodLabel || paymentMethodLabel(sale.paymentMethod))}</td>
...
els.salesFilterDate.addEventListener('change', () => setSalesDateFilter(els.salesFilterDate.value));
els.salesTodayBtn.addEventListener('click', () => setSalesDateFilter(todayISO()));
els.salesAllBtn.addEventListener('click', setSalesAllFilter);
els.detailSalesDayBtn.addEventListener('click', () => openDashboardDetail('salesDay'));
els.detailExpensesDayBtn.addEventListener('click', () => openDashboardDetail('expensesDay'));
els.detailCashBtn.addEventListener('click', () => openDashboardDetail('cash'));
els.detailSalesMonthBtn.addEventListener('click', () => openDashboardDetail('salesMonth'));
...
els.closeDetailBtn.addEventListener('click', () => closeModal(els.detailModal));
...
[els.settingsModal, els.reportModal, els.detailModal].forEach((modal) => {
...
els.closeCajaForm.addEventListener('submit', async (e) => {
  ...
  await updateDoc(doc(db, 'cashSessions', state.currentSession.id), {
    ...
    closingDebitCard: box.tarjetaDebito,
    closingCreditCard: box.tarjetaCredito,
    closingTotal: total,
...
els.cashMovementForm.addEventListener('submit', async (e) => {
  ...
  const form = e.currentTarget;
  const existingMovement = form.id.value ? state.cashMovements.find((item) => item.id === form.id.value) : null;
  const payload = {
    date: form.date.value || todayISO(),
    ...
    cashSessionId: existingMovement?.cashSessionId || state.currentSession.id,
  };
  if (form.id.value) {
...
els.saleForm.addEventListener('submit', async (e) => {
  ...
  const existingSale = form.id.value ? state.sales.find((item) => item.id === form.id.value) : null;
  const payload = {
    date: form.date.value || todayISO(),
    amount: parseMoneyValue(form.amount.value),
    ...
    details: form.details.value.trim(),
    cashSessionId: existingSale?.cashSessionId || state.currentSession.id,
  };
...
els.expenseForm.addEventListener('submit', async (e) => {
  ...
  if (!requireOpenCaja()) return;
  const form = e.currentTarget;
  const existingExpense = form.id.value ? state.expenses.find((item) => item.id === form.id.value) : null;
  const payload = {
    date: form.date.value || todayISO(),
    amount: parseMoneyValue(form.amount.value),
    category: form.category.value,
    details: form.details.value.trim(),
    cashSessionId: existingExpense?.cashSessionId || state.currentSession.id,
  };
  if (form.id.value) {
...
if ('serviceWorker' in navigator) {
  window.addEventListener('load', async () => {
    try {
      ...
      regs.forEach((reg) => {
        if (!reg.active?.scriptURL?.includes('sf-final-20260406-detalles-inicio') && !reg.installing?.scriptURL?.includes('sf-final-20260406-detalles-inicio')) {
          reg.update();
        }
      });
      await navigator.serviceWorker.register('./sw.js?v=sf-final-20260406-detalles-inicio');
    } catch (error) {
      console.error(error);
    }
  });
...
function init() {
  ...
  state.salesFilterDate = todayISO();
  els.salesFilterDate.value = state.salesFilterDate;
  els.openCajaForm.date.value = todayISO();
