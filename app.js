...
function renderFloatingNotices() {
  ...
  els.floatingNoticeList.innerHTML = items.map((msg) => `
    <div class="simple-item notice-item">
      <div>
        <strong>${msg.text}</strong>
        <span>${msg.authorName || msg.authorUsername || "-"}</span>
      </div>
      ${isAdmin() ? `<button class="btn btn-danger delete-floating-notice-btn" data-id="${msg.id}" type="button">Eliminar</button>` : ""}
    </div>
  `).join("");

  document.querySelectorAll(".delete-floating-notice-btn").forEach((btn) => {
...
async function ensureAnonymousSession() {
  ...
  try {
    ...
    console.error(error);
    throw new Error("No se pudo conectar con Firebase para iniciar sesión. Revisá Firebase Authentication > Anonymous, las reglas de Firestore y que el sitio publicado sea el correcto.");
  }
}

async function countAdminUsers() {
  const snap = await getDocs(collection(db, "users"));
  return snap.docs.filter((item) => item.data()?.role === "administrador").length;
}
...
async function findUserByCredentials(username, password, role) {
  ...
  if (String(data.username || "").trim().toLowerCase() !== cleanUsername) return null;
