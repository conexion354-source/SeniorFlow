...
function setDefaultDates() {
  ["saleForm", "expenseForm", "paymentForm", "budgetForm"].forEach((id) => {
...
function createBudgetItemRow(item = {}) {
  ...
  if (source === "stock" && item.stockId) {
    ...
    if (product && !item.unitPrice) row.querySelector('[name="unitPrice"]').value = Number(product.cost || 0);
  }
  ...
  stockSelect?.addEventListener("change", () => {
    ...
    row.querySelector('[name="unitPrice"]').value = Number(product.cost || 0);
    updateBudgetTotals();
  });
...
function renderFloatingNotices() {
  ...
  if (!items.length) {
    els.floatingNoticeList.innerHTML = '<div class="empty">No hay avisos activos.</div>';
    return;
  }
  ...
  els.budgetPreviewCard.innerHTML = `
    <div class="budget-sheet">
      <div class="budget-sheet-head">
        <div>
          <h2>Mundo Led Chaco</h2>
          <p>Av. San Martín 646 · Cel: 3735 506858</p>
          <p>info@mundoledchaco.com · www.mundoledchaco.com</p>
        </div>
        <div class="budget-sheet-side">
          <strong>Presupuesto ${budget.number || budget.id}</strong>
          <span>Fecha: ${budget.date || "-"}</span>
          <span>Validez: ${budget.validUntil || "-"}</span>
        </div>
      </div>

      <div class="budget-client-box">
        <strong>Cliente: ${budget.clientName || "-"}</strong>
        <span>Celular: ${budget.clientPhone || "-"}</span>
        <span>Email: ${budget.clientEmail || "-"}</span>
        <span>Dirección: ${budget.clientAddress || "-"}</span>
      </div>
...
function renderEverything() {
  ...
  renderSimpleList(
    ...
    state.budgets,
    (budget) => `
      <button class="simple-item budget-btn ${state.selectedBudgetId === budget.id ? "active" : ""}" data-id="${budget.id}" type="button">
        <strong>${budget.number || budget.id} · ${budget.clientName || "Cliente"}</strong>
        <span>${currency(budget.total)}</span>
        <div class="muted">${budgetPaymentLabel(budget)}${budget.convertedToSale ? " · Convertido en venta" : ""}</div>
      </button>
    `,
