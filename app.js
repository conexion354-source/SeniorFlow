...
function integerToSpanishWords(value) {
  ...
  const prefix = millions === 1 ? "un millón" : `${integerToSpanishWords(millions)} millones`;
  return rest ? `${prefix} ${integerToSpanishWords(rest)}` : prefix;
}
...
function moneyToWords(value) {
  ...
  const pesosText = `${integerToSpanishWords(whole)} ${whole === 1 ? "peso" : "pesos"}`;
  if (!cents) return pesosText.charAt(0).toUpperCase() + pesosText.slice(1);
  const centsText = `${integerToSpanishWords(cents)} ${cents === 1 ? "centavo" : "centavos"}`;
  const finalText = `${pesosText} con ${centsText}`;
...
function enhanceMoneyInputs(root = document) {
  root.querySelectorAll('input[data-money="true"]').forEach((input) => {
    ...
    const updateText = () => {
      ...
      const target = label?.querySelector('.money-text') || document.querySelector(`.money-text[data-money-text-for="${input.name}"]`);
      if (target) target.textContent = moneyToWords(input.value || 0);
    };
    ...
    input.addEventListener("blur", () => {
      input.value = formatMoneyInputValue(input.value || 0);
      updateText();
      if (input.closest('.budget-item-row')) updateBudgetTotals();
    });
