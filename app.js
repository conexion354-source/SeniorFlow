...
function setupInstallPrompt() {
  const showInstallHelp = () => {
    ...
    if (isIOS) {
      alert("Para instalar en iPhone o iPad: abrí Compartir y elegí 'Agregar a pantalla de inicio'.");
      return;
    }
    alert("Si no aparece la ventana de instalación, abrí el menú del navegador (⋮) y elegí 'Instalar aplicación' o 'Agregar a pantalla de inicio'.");
  };
  ...
  window.addEventListener("appinstalled", () => {
    ...
    deferredPrompt = null;
    setTimeout(() => els.installBtn.classList.add("hidden"), 1500);
  });
