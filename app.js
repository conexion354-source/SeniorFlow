...
function openBudgetPreviewModal(budgetId = state.selectedBudgetId) {
  renderBudgetPreview(budgetId);
  if (!els.budgetPreviewModal) return;
  els.budgetPreviewModal.classList.remove("hidden");
  els.budgetPreviewModal.setAttribute("aria-hidden", "false");
  document.body.classList.add("modal-open");
}

function closeBudgetPreviewModal() {
  if (!els.budgetPreviewModal) return;
  els.budgetPreviewModal.classList.add("hidden");
  els.budgetPreviewModal.setAttribute("aria-hidden", "true");
...
function renderBudgetPreview(budgetId = state.selectedBudgetId) {
  ...
  if (!budget) {
    els.budgetPreviewCard.innerHTML = '<div class="empty">Seleccioná un presupuesto para verlo.</div>';
    ...
    if (els.budgetModalTitle) els.budgetModalTitle.textContent = "Vista previa del presupuesto";
    return;
  }
  ...
  if (els.budgetModalTitle) {
    els.budgetModalTitle.textContent = `Presupuesto ${budget.number || budget.id} · ${budget.clientName || "Cliente"}`;
  }
      ...
      <div class="budget-sheet-head">
        ...
        <div class="budget-sheet-side">
          ...
          <div class="top-badges">
            ${budget.sent ? '<span class="status-pill success">Enviado</span>' : '<span class="status-pill">Pendiente</span>'}
...
function renderBudgets() {
  ...
  els.budgetsList.innerHTML = filteredBudgets.length ? filteredBudgets.map((budget) => `
    <article class="budget-list-card ${state.selectedBudgetId === budget.id ? "active" : ""}">
      <div class="budget-list-card-top">
        <div class="budget-list-card-main">
          <strong>${escapeHtml(budget.number || budget.id)} · ${escapeHtml(budget.clientName || "Cliente")}</strong>
          <div class="budget-list-card-meta">
            <span>${escapeHtml(budget.date || "-")}</span>
            <span>${escapeHtml(budgetPaymentLabel(budget))}</span>
          </div>
          <div class="top-badges">
            ${budget.sent ? '<span class="status-pill success">Enviado</span>' : '<span class="status-pill">Pendiente</span>'}
            ${budget.approved ? '<span class="status-pill success">Aprobado</span>' : ''}
        ...
        <div class="budget-list-amount">${currency(budget.total)}</div>
      </div>

      <div class="budget-list-card-footer">
        <span class="muted">${(budget.items || []).length} ítem${(budget.items || []).length === 1 ? "" : "s"}</span>
...
function updateBudgetRowTotal(row) {
  if (!row) return;
  const quantity = Number(row.querySelector('[name="quantity"]')?.value || 0);
  const unit = row.querySelector('[name="unit"]')?.value || "unidad";
  const unitPrice = parseMoneyValue(row.querySelector('[name="unitPrice"]')?.value || 0);
  const total = quantity * unitPrice;
  const totalNode = row.querySelector('[data-budget-line-total]');
  const summaryNode = row.querySelector('[data-budget-line-summary]');
  if (totalNode) totalNode.textContent = currency(total);
  if (summaryNode) summaryNode.textContent = `${quantity || 0} ${unit}`;
}

function updateBudgetTotals() {
  const rows = [...(els.budgetItemsWrap?.querySelectorAll('.budget-item-row') || [])];
  rows.forEach(updateBudgetRowTotal);
...
function createBudgetItemRow(item = {}) {
  ...
  row.innerHTML = `
    <div class="budget-item-top">
      <div>
        <span class="budget-item-label">Ítem</span>
        <strong class="budget-item-title">${source === "stock" ? "Producto desde stock" : "Ítem manual"}</strong>
      </div>
      <div class="budget-item-top-actions">
        <label class="budget-source-field compact">
          <span>Origen</span>
          <select name="source">
    ...
    <div class="budget-item-fields">
      ...
      <div class="budget-line-total-box">
        <span>Total</span>
        <strong data-budget-line-total>${currency((Number(item.quantity || 1) || 0) * Number(item.unitPrice || 0))}</strong>
        <small data-budget-line-summary>${item.quantity || 1} ${escapeHtml(item.unit || "unidad")}</small>
      </div>
    </div>
  `;
  ...
  const titleNode = row.querySelector('.budget-item-title');
  ...
  sourceSelect?.addEventListener('change', () => {
    const isStock = sourceSelect.value === 'stock';
    stockWrap.classList.toggle('hidden', !isStock);
    if (titleNode) titleNode.textContent = isStock ? 'Producto desde stock' : 'Ítem manual';
    if (isStock) applyProduct();
    updateBudgetTotals();
  });
...
function setupUiEvents() {
  ...
  els.budgetPreviewCloseBtn?.addEventListener("click", closeBudgetPreviewModal);
  els.budgetModalEditBtn?.addEventListener("click", () => {
    if (!state.selectedBudgetId) return;
    loadBudgetIntoForm(state.selectedBudgetId);
  });
  els.budgetPreviewModal?.querySelectorAll("[data-close-budget-modal]").forEach((node) => node.addEventListener("click", closeBudgetPreviewModal));
  document.addEventListener("keydown", (event) => {
    if (event.key === "Escape" && !els.budgetPreviewModal?.classList.contains("hidden")) closeBudgetPreviewModal();
  });
...
async function saveBudgetForm(event) {
  ...
  if (!els.budgetPreviewModal?.classList.contains("hidden")) renderBudgetPreview(id);
  resetBudgetForm();
