...
export default function App() {
  ...
  const parseMontoCaja = (valor) => parseFloat((valor ?? '').toString().replace(',', '.')) || 0;
  ...
  const abrirFormularioUsuario = (usuario = null) => {
    ...
    setFormUsuario({
      ...
      rol: usuario?.rol === 'admin' ? 'admin' : 'cajero'
    });
  ...
  const eliminarCategoriaInventario = async (categoria) => {
    ...
    try {
      ...
      if (configExportInventario.categoria === categoria) {
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
              ...
              <div className="px-4 py-4 border-b border-gray-100 bg-white">
                <div className="bg-slate-50 border border-slate-200 rounded-xl p-3 space-y-3">
                  <p className="text-[10px] font-bold text-slate-600 uppercase tracking-wider">Gestión rápida de categorías y marcas</p>
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
                    <div className="bg-white border border-indigo-100 rounded-xl p-3">
                      <p className="text-[10px] font-bold text-indigo-700 uppercase tracking-wider mb-2">Categorías</p>
                      {categoriasInventario.length === 0 ? (
                        <p className="text-xs font-bold text-gray-400">Sin categorías cargadas.</p>
                      ) : (
                        <div className="flex flex-wrap gap-2">
                          {categoriasInventario.map((cat) => {
                            ...
                            return (
                              <span key={cat} className="inline-flex items-center gap-1.5 px-2 py-1 rounded-lg border border-indigo-200 bg-indigo-50 text-indigo-800 text-xs font-bold">
                                {cat}
                                <button
                                  type="button"
                                  onClick={() => eliminarCategoriaInventario(cat)}
                                  disabled={isBusy || Boolean(gestionTaxonomiaBusy)}
                                  className="w-5 h-5 rounded-md bg-white border border-indigo-200 text-red-600 hover:bg-red-50 disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center transition-colors"
                                  title="Quitar categoría de todos los productos"
                                >
                                  <X size={12} />
                                </button>
                              </span>
                    ...
                    <div className="bg-white border border-purple-100 rounded-xl p-3">
                      <p className="text-[10px] font-bold text-purple-700 uppercase tracking-wider mb-2">Marcas</p>
                      {marcasInventario.length === 0 ? (
                        <p className="text-xs font-bold text-gray-400">Sin marcas cargadas.</p>
                      ) : (
                        <div className="flex flex-wrap gap-2">
                          {marcasInventario.map((marca) => {
                            ...
                            const isBusy = gestionTaxonomiaBusy === busyKey;
                            return (
                              <span key={marca} className="inline-flex items-center gap-1.5 px-2 py-1 rounded-lg border border-purple-200 bg-purple-50 text-purple-800 text-xs font-bold">
                                {marca}
                                <button
                                  type="button"
                                  onClick={() => eliminarMarcaInventario(marca)}
                                  disabled={isBusy || Boolean(gestionTaxonomiaBusy)}
                                  className="w-5 h-5 rounded-md bg-white border border-purple-200 text-red-600 hover:bg-red-50 disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center transition-colors"
                                  title="Quitar marca de todos los productos"
                                >
                                  <X size={12} />
                                </button>
                              </span>
