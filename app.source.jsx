...
export default function App() {
  ...
  const [formProducto, setFormProducto] = useState({ codigo: '', categoria: '', descripcion: '', costo: '', ganancia: '', iva: '21', precio: '', unidad: 'unid', cantidad: '', imagen: '' });
  ...
  const productosVisualizados = useMemo(() => {
    ...
    return productos.filter(p => p.descripcion.toLowerCase().includes(busquedaLower) || (p.codigo && p.codigo.toLowerCase().includes(busquedaLower)) || (p.categoria && p.categoria.toLowerCase().includes(busquedaLower)));
  ...
  const guardarProducto = async (e) => {
    ...
    setFormProducto({ codigo: '', categoria: '', descripcion: '', costo: '', ganancia: '', iva: '21', precio: '', unidad: 'unid', cantidad: '', imagen: '' });
  };
  ...
  const descargarListaExcelInventario = (items, alcanceLabel) => {
    const encabezados = ['Codigo', 'Categoria', 'Descripcion', 'Unidad', 'Stock', 'Costo', 'GananciaPct', 'IVA', 'Precio'];
  ...
  const obtenerNumeroPresupuestoTexto = (presupuesto) => {
    ...
    if (presupuesto?.id) return `N° ${presupuesto.id.slice(-6).toUpperCase()}`;
    return 'N° -';
  };
    ...
    <div className="min-h-screen bg-slate-50 font-sans pb-28 md:pb-8 print:bg-white print:pb-0">
      ...
      {modalActivo === 'imprimir_catalogo' && catalogoAImprimir && (
        <div className="fixed inset-0 bg-gray-100 z-50 overflow-y-auto flex justify-center py-10 print:p-0 print:bg-white custom-scrollbar">
          ...
          <div className="bg-white w-full max-w-[210mm] min-h-[297mm] shadow-2xl print:shadow-none p-8 sm:p-12 text-black font-sans">
            <div className="flex justify-between items-start border-b-2 border-gray-800 pb-6 mb-6">
              <div className="flex gap-4 items-center">
                ...
                <div>
                  ...
                  <p className="text-sm font-bold text-gray-500 mt-1 uppercase">Catálogo Comercial</p>
                </div>
              </div>
              <div className="text-right">
                <h2 className="text-2xl font-black text-gray-300 tracking-widest mb-1">CATÁLOGO</h2>
                <p className="font-bold text-sm">FECHA: {new Date().toLocaleDateString('es-AR')}</p>
                <p className="text-[10px] font-bold text-gray-500 uppercase mt-1">Alcance: {catalogoAImprimir.alcanceLabel}</p>
              </div>
            </div>

            <div className="space-y-6">
              {catalogoAImprimir.grupos.map((grupo) => (
                <div key={grupo.categoria} className="break-inside-avoid-page">
                  <div className="bg-slate-800 text-white px-4 py-2 rounded-lg mb-3">
                    <p className="text-xs font-black uppercase tracking-wider">{grupo.categoria}</p>
                  </div>
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                    {grupo.items.map((p) => (
                      <div key={p.id} className="border border-gray-200 rounded-xl overflow-hidden bg-white break-inside-avoid-page">
                        ...
                        <div className="p-3">
                          <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-1">Cód: {p.codigo || '-'}</p>
                          <p className="font-bold text-gray-900 text-[11px] leading-snug min-h-[34px]">{p.descripcion}</p>
                          <div className="mt-2 flex justify-between items-end gap-2">
                            ...
                            <span className="text-base font-black text-indigo-700">{formatearDinero(p.precio)}</span>
                          </div>
                        </div>
                      </div>
                    ))}
      ...
      {modalActivo === 'nuevo_producto' && (
        <Modal titulo={productoAEditar ? 'Editar Producto' : 'Nuevo Producto'} onClose={() => { setModalActivo(null); setProductoAEditar(null); }}>
          <form onSubmit={guardarProducto} className="space-y-4">
            ...
            <div><label className="block text-[10px] font-bold text-gray-500 mb-1 uppercase tracking-wider">Categoría</label><input type="text" value={formProducto.categoria} onChange={(e) => setFormProducto({...formProducto, categoria: e.target.value})} className="w-full px-3 py-2.5 bg-white border border-gray-200 rounded-lg focus:ring-2 focus:ring-indigo-600 outline-none text-sm font-bold" placeholder="Ej: Pinturas / Herramientas / Iluminación"/></div>
            ...
            <table className="w-full text-sm mb-8 border-collapse">
              <thead className="bg-gray-800 text-white font-bold uppercase text-[10px] tracking-wider">
                <tr>
                  ...
                  <th className={`py-2 px-3 text-left ${incluirImagenesPdf ? 'w-[31%]' : 'w-[43%]'}`}>Descripción del Producto/Servicio</th>
                  ...
                  {mostrarDescuentoItemEnPdf && <th className="py-2 px-3 text-center w-[8%]">Desc.%</th>}
                ...
                {presupuestoAImprimir.items.map((item, i) => {
                  ...
                  return (
                    <tr key={i} className="border-b border-gray-200">
                      ...
                      {mostrarDescuentoItemEnPdf && <td className="py-2 px-3 text-center font-bold text-[11px] align-top">{parseNumeroPresupuesto(item?.descuento) > 0 ? `${parseNumeroPresupuesto(item?.descuento)}%` : '-'}</td>}
                      <td className="py-2 px-3 text-right font-black text-[11px] align-top">{formatearDinero(calcularTotalItemPresupuesto(item))}</td>
                    </tr>
                  );
                })}
              </tbody>
              ...
              <div className="w-1/2 flex justify-end">
                <div className="bg-gray-100 p-6 rounded-xl border-2 border-gray-800 min-w-[300px] text-right">
                  {resumenPresupuestoImpresion.descuentoItems > 0 && (
                    <p className="text-[10px] font-bold text-gray-500 uppercase mb-1">Descuento por Ítems: -{formatearDinero(resumenPresupuestoImpresion.descuentoItems)}</p>
                  )}
                  {resumenPresupuestoImpresion.descuentoGeneralMonto > 0 && (
                    <p className="text-[10px] font-bold text-gray-500 uppercase mb-1">Desc. General ({resumenPresupuestoImpresion.descuentoGeneralPct}%): -{formatearDinero(resumenPresupuestoImpresion.descuentoGeneralMonto)}</p>
                  )}
                  <p className="text-sm font-bold text-gray-600 uppercase tracking-widest mb-1">TOTAL PRESUPUESTO</p>
                  <p className="text-4xl font-black text-gray-900 tracking-tight">{formatearDinero(resumenPresupuestoImpresion.total)}</p>
                  <p className="text-[10px] font-bold text-gray-500 uppercase mt-2">{numeroALetras(resumenPresupuestoImpresion.total)}</p>
                </div>
              </div>
            </div>
