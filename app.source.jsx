...
export default function App() {
  ...
  const registrarMovimiento = async (e, tipo) => {
    ...
    if (formData.metodoPago === 'cuenta_corriente' && (tipo === 'venta' || tipo === 'ingreso_extra')) {
      ...
      if (clienteExistente) {
        ...
        await addDoc(collection(db, 'clientes'), { numero: obtenerSiguienteNumeroCliente(), nombre: nombreCliente, whatsapp: formData.detallesPago.whatsapp || '', saldo: monto });
      }
    }
  ...
  const generarPdfPresupuestoFile = (p) => {
    ...
    const numeroArchivo = parseEnteroPresupuesto(p?.numero);
    const nroArchivo = numeroArchivo > 0
      ? String(numeroArchivo).padStart(6, '0')
      : normalizarTextoArchivo(numeroPresupuesto.replace(/^N°\s*/i, ''));
                    ...
                    <div key={cliente.id} className="p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-purple-50/30 transition-colors">
                      ...
                      <div className="flex items-center gap-4 justify-between sm:justify-end border-t sm:border-0 pt-3 sm:pt-0 border-gray-100">
                        ...
                        <div className="flex items-center gap-2">
                          <button onClick={() => abrirFormularioCliente(cliente)} className="bg-white border border-purple-200 text-purple-700 hover:bg-purple-50 px-3 py-2.5 rounded-xl font-bold text-sm transition-all flex items-center gap-1.5 shadow-sm hover:shadow-md active:scale-95 uppercase tracking-wider">
