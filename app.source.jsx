...
export default function App() {
  ...
  const renombrarCategoriaInventario = async (categoria) => {
    ...
    const propuesto = window.prompt(`Nuevo nombre para la categoría "${categoria}":`, categoria);
    if (propuesto === null) return;
    const nuevoNombre = propuesto.trim();
    if (!nuevoNombre) {
      alert('El nombre no puede quedar vacío.');
      return;
    }
    const nuevoNorm = normalizarTaxonomia(nuevoNombre);
    if (nuevoNorm === categoriaNorm) return;

    const confirmar = window.confirm(`Se renombrará "${categoria}" a "${nuevoNombre}" en ${afectados.length} producto(s). ¿Continuar?`);
    if (!confirmar) return;
  ...
  const renombrarMarcaInventario = async (marca) => {
    ...
    const nuevoNorm = normalizarTaxonomia(nuevoNombre);
    if (nuevoNorm === marcaNorm) return;

    const confirmar = window.confirm(`Se renombrará "${marca}" a "${nuevoNombre}" en ${afectados.length} producto(s). ¿Continuar?`);
    if (!confirmar) return;
              ...
              <div>
                ...
                <div className="mt-1.5 border border-indigo-100 bg-indigo-50/40 rounded-lg max-h-28 overflow-y-auto">
                  {(() => {
                    ...
                    const visibles = categoriasInventario.filter((cat) => normalizarTaxonomia(cat).includes(filtro));
                    if (!visibles.length) {
                    ...
                    return visibles.map((cat, idx) => {
                      ...
                      return (
                        <div key={cat} className={`flex items-center justify-between gap-2 px-2.5 py-1.5 ${idx < visibles.length - 1 ? 'border-b border-indigo-100' : ''}`}>
                          <button
                            type="button"
                          ...
                          <div className="flex items-center gap-1 shrink-0">
                            <button
                              type="button"
                              onClick={() => renombrarCategoriaInventario(cat)}
              ...
              <div>
                ...
                <div className="mt-1.5 border border-purple-100 bg-purple-50/40 rounded-lg max-h-28 overflow-y-auto">
                  {(() => {
                    ...
                    const visibles = marcasInventario.filter((marca) => normalizarTaxonomia(marca).includes(filtro));
                    if (!visibles.length) {
                      return <p className="px-3 py-2 text-[11px] font-bold text-gray-400">Sin coincidencias para ese texto.</p>;
                    }
                    return visibles.map((marca, idx) => {
                      ...
                      const isBusy = gestionTaxonomiaBusy === busyKey;
                      return (
                        <div key={marca} className={`flex items-center justify-between gap-2 px-2.5 py-1.5 ${idx < visibles.length - 1 ? 'border-b border-purple-100' : ''}`}>
                          <button
                            type="button"
                          ...
                          <div className="flex items-center gap-1 shrink-0">
                            <button
                              type="button"
                              onClick={() => renombrarMarcaInventario(marca)}
                              disabled={isBusy || Boolean(gestionTaxonomiaBusy)}
