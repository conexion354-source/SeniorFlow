...
const appId = (typeof window !== 'undefined' && window.__app_id) ? window.__app_id : 'mi-negocio-app';
...
export default function App() {
  ...
  const [formProducto, setFormProducto] = useState({ codigo: '', descripcion: '', costo: '', ganancia: '', iva: '21', precio: '', unidad: 'unid', cantidad: '', imagen: '' });
  ...
  useEffect(() => {
    ...
    const unsubConfig = onSnapshot(doc(db, 'artifacts', appId, 'public', 'data', 'sistema', 'configuracion'), (d) => {
        if(d.exists()) setConfiguracion(d.data());
        else setDoc(d.ref, { nombre: 'Mi Negocio', logo: '' });
    }, (err) => console.error(err));

    const unsubCaja = onSnapshot(doc(db, 'artifacts', appId, 'public', 'data', 'sistema', 'caja'), (d) => {
        if(d.exists()) setCaja(d.data());
        else setDoc(d.ref, { estado: 'cerrada', efectivoInicial: 0, chequesInicial: 0, fechaApertura: null });
    }, (err) => console.error(err));

    const unsubUsuarios = onSnapshot(collection(db, 'artifacts', appId, 'public', 'data', 'usuarios'), (snapshot) => {
        if (snapshot.empty) {
            addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'usuarios'), { nombre: 'Admin Principal', username: 'admin', password: '123', rol: 'admin' });
        } else {
            const loaded = []; snapshot.forEach(doc => loaded.push({ id: doc.id, ...doc.data() })); setUsuarios(loaded);
        }
    }, (err) => console.error(err));

    const unsubMovs = onSnapshot(collection(db, 'artifacts', appId, 'public', 'data', 'movimientos'), (snapshot) => {
    ...
    const unsubClientes = onSnapshot(collection(db, 'artifacts', appId, 'public', 'data', 'clientes'), (snapshot) => {
        const loaded = []; snapshot.forEach(doc => loaded.push({ id: doc.id, ...doc.data() })); setClientes(loaded);
    }, (err) => console.error(err));

    const unsubPresupuestos = onSnapshot(collection(db, 'artifacts', appId, 'public', 'data', 'presupuestos'), (snapshot) => {
        const loaded = []; snapshot.forEach(doc => loaded.push({ id: doc.id, ...doc.data() })); 
        loaded.sort((a, b) => new Date(b.fecha) - new Date(a.fecha));
        setPresupuestos(loaded);
    }, (err) => console.error(err));

    const unsubProductos = onSnapshot(collection(db, 'artifacts', appId, 'public', 'data', 'productos'), (snapshot) => {
        const loaded = []; snapshot.forEach(doc => loaded.push({ id: doc.id, ...doc.data() })); 
  ...
  useEffect(() => {
    if (modalActivo === 'scanner_codigo') {
      const initScanner = () => {
        if (!window.Html5QrcodeScanner) return;
        const scanner = new window.Html5QrcodeScanner("reader", { fps: 10, qrbox: {width: 250, height: 150} }, false);
        scanner.render((decodedText) => {
          setFormProducto(prev => ({ ...prev, codigo: decodedText }));
          scanner.clear();
  ...
  const datosReporte = useMemo(() => {
    ...
    if (reporteTiempo === 'hoy') { inicio = new Date(); inicio.setHours(0,0,0,0); } 
    else if (reporteTiempo === 'semana') { inicio = new Date(); inicio.setDate(inicio.getDate() - inicio.getDay() + (inicio.getDay() === 0 ? -6 : 1)); inicio.setHours(0,0,0,0); } 
    else if (reporteTiempo === 'mes') { inicio = new Date(ahora.getFullYear(), ahora.getMonth(), 1); inicio.setHours(0,0,0,0); }

    const movsFiltrados = movimientos.filter(m => { const mDate = new Date(m.fecha); return mDate >= inicio && mDate <= ahora; });
  ...
  const productosVisualizados = useMemo(() => {
    ...
    return productos.filter(p => p.descripcion.toLowerCase().includes(busquedaLower) || (p.codigo && p.codigo.toLowerCase().includes(busquedaLower)));
  ...
  const abrirCaja = async (e) => {
    e.preventDefault();
    const docRef = doc(db, 'artifacts', appId, 'public', 'data', 'sistema', 'caja');
  ...
  const guardarConfiguracion = async (e) => {
    e.preventDefault();
    await setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'sistema', 'configuracion'), configuracion);
  ...
  const registrarMovimiento = async (e, tipo) => {
    ...
    if (formData.metodoPago === 'cuenta_corriente' && (tipo === 'venta' || tipo === 'ingreso_extra')) {
      ...
      if (clienteExistente) {
        await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'clientes', clienteExistente.id), { saldo: clienteExistente.saldo + monto, whatsapp: formData.detallesPago.whatsapp || clienteExistente.whatsapp });
      } else {
        await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'clientes'), { nombre: nombreCliente, whatsapp: formData.detallesPago.whatsapp || '', saldo: monto });
      }
    }
  ...
  const registrarCobro = async (e) => {
    ...
    await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'clientes', clienteSeleccionado.id), { saldo: clienteSeleccionado.saldo - monto });
    await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'movimientos'), {
  ...
  const ejecutarEliminacion = async () => {
    ...
    if (mov.tipo === 'cobro' && mov.detallesPago?.clienteId) {
      ...
      if (cliente) await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'clientes', cliente.id), { saldo: cliente.saldo + mov.monto });
    }
    ...
    await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'movimientos', mov.id));
  ...
  const guardarEdicionMovimiento = async (e) => {
    ...
    if (movOriginal.tipo === 'cobro' && movOriginal.detallesPago?.clienteId) {
        ...
        if (cliente) await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'clientes', cliente.id), { saldo: cliente.saldo + movOriginal.monto });
    }
    ...
    if (movOriginal.tipo === 'cobro' && movOriginal.detallesPago?.clienteId) {
        const cliente = clientes.find(c => c.id === movOriginal.detallesPago.clienteId);
        if (cliente) await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'clientes', cliente.id), { saldo: cliente.saldo - montoEditado });
        ...
        if (cliente) {
            await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'clientes', cliente.id), { saldo: cliente.saldo + montoEditado });
        } else {
            await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'clientes'), { nombre: nombreNuevo, whatsapp: formData.detallesPago?.whatsapp || '', saldo: montoEditado });
        }
    }

    await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'movimientos', movimientoAEditar.id), {
        monto: montoEditado,
  ...
  const guardarUsuario = async (e) => {
    e.preventDefault();
    if (usuarioAEditar) {
      await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'usuarios', usuarioAEditar.id), { ...formUsuario });
    } else {
      await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'usuarios'), { ...formUsuario });
    }
  ...
  const guardarProducto = async (e) => {
    ...
    if (productoAEditar) {
      await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'productos', productoAEditar.id), data);
    } else {
      await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'productos'), data);
    }
    setModalActivo(null); setProductoAEditar(null);
    setFormProducto({ codigo: '', descripcion: '', costo: '', ganancia: '', iva: '21', precio: '', unidad: 'unid', cantidad: '', imagen: '' });
  };

  const eliminarProducto = async (id) => {
    if (window.confirm("¿Seguro que deseas eliminar este producto del inventario?")) {
      await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'productos', id));
    }
  };
  ...
  const agregarItemPresupuesto = () => {
    setFormPresupuesto({
      ...formPresupuesto,
      items: [...formPresupuesto.items, { id: Date.now(), codigo: '', descripcion: '', unidad: 'unid', precio: '', cantidad: 1 }]
    });
  };
  ...
  const guardarPresupuesto = async (e) => {
    ...
    const total = formPresupuesto.items.reduce((acc, item) => acc + (parseFloat(item.precio || 0) * parseFloat(item.cantidad || 0)), 0);
    ...
    if (formPresupuesto.esNuevoCliente) {
      ...
      const docRef = await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'clientes'), { nombre: cNombre, whatsapp: cWpp, saldo: 0 });
      cId = docRef.id;
    } else {
    ...
    const data = {
      clienteId: cId, clienteNombre: cNombre, whatsapp: cWpp, items: formPresupuesto.items, 
      total, estado: formPresupuesto.estado, notas: formPresupuesto.notas,
      fecha: formPresupuesto.id ? formPresupuesto.fecha : new Date().toISOString(), usuario: usuarioActual.nombre
    };

    if (formPresupuesto.id) {
      await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'presupuestos', formPresupuesto.id), data);
    } else {
      await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'presupuestos'), data);
    }
    ...
    <div className="min-h-screen bg-slate-50 font-sans pb-28 md:pb-8 print:bg-white print:pb-0">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
              <div className="p-4 border-b border-gray-100 bg-gray-50/50 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                ...
                <div className="relative w-full sm:w-auto">
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400"><Search size={16} /></span>
                  <input 
                    type="text" placeholder="Buscar producto o código..." value={busquedaInventario} onChange={(e) => setBusquedaInventario(e.target.value)}
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {productosVisualizados.map((p) => (
                        <tr key={p.id} className="hover:bg-indigo-50/50 transition-colors group">
                          ...
                          <td className="px-5 py-3 font-bold text-gray-900">{p.descripcion}</td>
                          ...
                          <td className="px-5 py-3 text-right">
                             <div className="opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity flex gap-1 justify-end">
                                <button onClick={() => { setProductoAEditar(p); setFormProducto({ codigo: p.codigo || '', descripcion: p.descripcion, costo: p.costo || '', ganancia: p.ganancia || '', iva: p.iva || '21', precio: p.precio, unidad: p.unidad || 'unid', cantidad: p.cantidad || 0, imagen: p.imagen || '' }); setModalActivo('nuevo_producto'); }} className="p-2 bg-white border shadow-sm text-blue-600 hover:bg-blue-50 hover:border-blue-200 rounded-lg transition-all" title="Editar"><Edit2 size={16} /></button>
        ...
        {puedeVerSistema && vista === 'presupuestos' && usuarioActual.rol === 'admin' && (
          <div className="space-y-6 animate-in fade-in duration-300 print:hidden">
            <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              ...
              <button onClick={() => { setFormPresupuesto({ id: null, esNuevoCliente: false, clienteId: '', clienteNombre: '', whatsapp: '', items: [], notas: '', estado: 'borrador' }); setModalActivo('nuevo_presupuesto'); }} className="bg-teal-600 hover:bg-teal-700 text-white font-bold py-2.5 px-4 rounded-xl shadow-sm transition-all active:scale-95 flex items-center justify-center gap-1.5 w-full sm:w-auto text-sm uppercase tracking-wider"><FilePlus2 size={16} /> Crear Presupuesto</button>
            </div>
        ...
        <Modal titulo="Opciones de Presupuesto" onClose={() => { setModalActivo(null); setPresupuestoSeleccionado(null); setIncluirImagenesPdf(false); }} customWidth="max-w-sm">
           ...
           <div className="space-y-4">
              ...
              <div className="border-t border-gray-100 pt-4 flex gap-2">
                <button onClick={() => { setFormPresupuesto(presupuestoSeleccionado); setModalActivo('nuevo_presupuesto'); }} className="flex-1 bg-white border border-gray-200 text-gray-700 hover:bg-gray-50 font-bold py-2 rounded-lg text-sm transition-colors flex justify-center items-center gap-1"><Edit2 size={16}/> Editar</button>
          ...
          <form onSubmit={guardarPresupuesto} className="space-y-4">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              <div className="bg-teal-50 p-3 rounded-xl border border-teal-200 flex flex-col justify-center items-end sm:w-48 shrink-0">
                 <p className="text-[10px] font-bold text-teal-800 uppercase mb-0.5">Total Cotización</p>
                 <p className="text-2xl font-black text-teal-700 tracking-tight">{formatearDinero(calcularTotalPresupuesto(formPresupuesto.items))}</p>
              </div>
            </div>
                   ...
                   <tbody className="divide-y divide-gray-100">
                     {formPresupuesto.items.map((item, index) => (
                       <tr key={index} className="hover:bg-slate-50 transition-colors group">
                         ...
                         <td className="p-2 text-right font-black text-sm text-teal-700 bg-teal-50/30">{formatearDinero((parseFloat(item.cantidad)||0) * (parseFloat(item.precio)||0))}</td>
      ...
      {modalActivo === 'imprimir_presupuesto' && presupuestoAImprimir && (
        <div className="fixed inset-0 bg-gray-100 z-50 overflow-y-auto flex justify-center py-10 print:p-0 print:bg-white custom-scrollbar">
          ...
          <div className="bg-white w-full max-w-[210mm] min-h-[297mm] shadow-2xl print:shadow-none p-10 sm:p-16 text-black font-sans relative">
            <div className="flex justify-between items-start border-b-2 border-gray-800 pb-8 mb-8">
              <div className="flex gap-4 items-center">
                 {configuracion.logo && <img src={configuracion.logo} alt="Logo" className="w-20 h-20 object-contain" />}
                 <div>
                   <h1 className="text-3xl font-black uppercase tracking-tight leading-none">{configuracion.nombre || 'MI NEGOCIO'}</h1>
                   <p className="text-sm font-bold text-gray-500 mt-1 uppercase">Presupuesto Formal</p>
                 </div>
              </div>
              <div className="text-right">
                <h2 className="text-3xl font-black text-gray-300 tracking-widest mb-1">PRESUPUESTO</h2>
            ...
            <div className="bg-gray-50 border border-gray-200 p-5 rounded-lg mb-8">
               ...
               <p className="text-xl font-black text-gray-900 uppercase">{presupuestoAImprimir.clienteNombre}</p>
               {presupuestoAImprimir.whatsapp && <p className="text-sm font-bold text-gray-600 mt-1">Tel/WhatsApp: {presupuestoAImprimir.whatsapp}</p>}
            </div>

            <table className="w-full text-sm mb-8 border-collapse">
              <thead className="bg-gray-800 text-white font-bold uppercase text-[10px] tracking-wider">
                <tr>
                  <th className="py-2 px-3 text-left w-[10%]">Cód.</th>
                  {incluirImagenesPdf && <th className="py-2 px-3 text-center w-[12%]">Imagen</th>}
                  <th className={`py-2 px-3 text-left ${incluirImagenesPdf ? 'w-[33%]' : 'w-[45%]'}`}>Descripción del Producto/Servicio</th>
                  <th className="py-2 px-3 text-center w-[10%]">Cant.</th>
                  <th className="py-2 px-3 text-center w-[10%]">Unid.</th>
                  <th className="py-2 px-3 text-right w-[12%]">Precio U.</th>
                  <th className="py-2 px-3 text-right w-[13%]">Subtotal</th>
                </tr>
              </thead>
                ...
                {presupuestoAImprimir.items.map((item, i) => {
                  ...
                  return (
                    <tr key={i} className="border-b border-gray-200">
                      <td className="py-2 px-3 text-gray-500 text-[11px] align-top">{item.codigo || '-'}</td>
                      {incluirImagenesPdf && (
                      ...
                      <td className="py-2 px-3 font-bold text-[11px] whitespace-normal break-words align-top leading-snug">{item.descripcion}</td>
                      <td className="py-2 px-3 text-center font-bold text-[11px] align-top">{item.cantidad}</td>
                      <td className="py-2 px-3 text-center text-[11px] uppercase align-top">{item.unidad}</td>
                      <td className="py-2 px-3 text-right text-[11px] align-top">{formatearDinero(item.precio)}</td>
                      <td className="py-2 px-3 text-right font-black text-[11px] align-top">{formatearDinero(item.cantidad * item.precio)}</td>
                    </tr>
                  );
                })}
              </tbody>
            ...
            <div className="flex justify-between items-start mt-8">
              <div className="w-1/2 pr-8">
                {presupuestoAImprimir.notas && (
                  <div><p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">Observaciones / Detalles:</p><p className="text-[11px] text-gray-700 whitespace-pre-wrap border-l-4 border-gray-200 pl-3">{presupuestoAImprimir.notas}</p></div>
                )}
              </div>
              <div className="w-1/2 flex justify-end">
                <div className="bg-gray-100 p-6 rounded-xl border-2 border-gray-800 min-w-[300px] text-right">
                  <p className="text-sm font-bold text-gray-600 uppercase tracking-widest mb-1">TOTAL PRESUPUESTO</p>
                  <p className="text-4xl font-black text-gray-900 tracking-tight">{formatearDinero(presupuestoAImprimir.total)}</p>
                  <p className="text-[10px] font-bold text-gray-500 uppercase mt-2">{numeroALetras(presupuestoAImprimir.total)}</p>
                </div>
              </div>
            </div>

            <div className="absolute bottom-10 left-10 right-10 border-t border-gray-300 pt-4 text-center">
               <p className="text-xs font-bold text-gray-400">Este documento es orientativo y está sujeto a modificaciones.</p>
               <p className="text-[10px] text-gray-400 mt-1">Emitido por: {presupuestoAImprimir.usuario} • {configuracion.nombre}</p>
            </div>
          </div>
        </div>
      )}
